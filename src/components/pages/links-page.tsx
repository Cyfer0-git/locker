...
export default function LinksPage() {
  ...
  const filteredLinks = useMemo(() => {
    ...
    return links.filter(
      l =>
        l.name.toLowerCase().includes(lowercasedTerm) ||
        l.url.toLowerCase().includes(lowercasedTerm)
    );
  }, [links, searchTerm]);
  ...
  return (
    <div className="mx-auto w-full h-full flex flex-col space-y-4">
      ...
      {links.length > 0 && filteredLinks.length === 0 && (
        <div className="text-center py-12 text-muted-foreground col-span-full">
          <p className="font-medium">No links found for "{searchTerm}"</p>
          <p className="text-sm">Try searching for something else.</p>
        </div>
      )}

      <AddLinkModal isOpen={isAddModalOpen} onOpenChange={setIsAddModalOpen} />
    </div>
  );
}
